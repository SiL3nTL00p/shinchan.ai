...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, isLatest = false }) => {
    ...
    return (
        ...
        >
            ...
            <div
                className={cn(
                    "max-w-[70%] rounded-2xl px-4 py-3 shadow-lg transition-all duration-200",
                    isUser
                        ? "bg-white text-[#1F2023] rounded-br-md"
                        : "bg-white text-black rounded-bl-md border border-[#444444]"
                )}
            >
                ...
                {/* Typing dots for latest AI message */}
                {!isUser && isLatest && (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}

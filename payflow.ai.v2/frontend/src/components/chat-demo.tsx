...
const DemoOne = () => {
    ...
    useEffect(() => {
        const saved = localStorage.getItem("payflow_conversations");
        const savedId = localStorage.getItem("payflow_current_id");
        if (saved) {
            try {
    ...
    useEffect(() => {
        if (conversations.length > 0) {
            localStorage.setItem("payflow_conversations", JSON.stringify(conversations));
        }
    }, [conversations]);

    useEffect(() => {
        if (currentConversationId) {
            localStorage.setItem("payflow_current_id", currentConversationId);
        }
